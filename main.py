...
def login_link():
    ...
    if code is not None:
        x.cookies.clear()
        code = code[0]
        _r = x.get(url=host + f"/P.aspx?authtype=1&code={code}&state=1", timeout=10)
        get_class_list()
        save_cookie(_r)
    else:
...
def login():
    ...
    x.cookies.clear()
    x.get(host, timeout=10)
    _r = x.post(url=host + "/AppCode/LoginInfo.ashx", data=params, headers=headers, timeout=10)
...
def sign(sign_code):
    if len(sign_code) == 4:
        ...
        _r = x.post(
            url=host + "/_CheckIn/CheckIn.ashx", data=params, headers=headers, timeout=10)
        if _r.status_code == 200:
            msg = _r.json()["msgbox"]
        ...
        _r = x.get(url=host + "/_CheckIn/MB/QrCodeCheckOK.aspx?state=" + sign_code, timeout=10)
        if _r.status_code == 200:
            soup = BeautifulSoup(_r.text, "lxml")
            ...
            if "签到成功" in msg:
                ...
                text_box.insert(tk.END, "\t非微信链接登录，二维码无法签到\n\n")
            return True
...
def sign_location(longitude, latitude):
    ...
    _r = x.post(
        url=host + "/_CheckIn/CheckInRoomHandler.ashx", data=params, headers=headers, timeout=10)
    if _r.status_code == 200:
        msg = _r.json()["msgbox"]
        text_box.insert(tk.END, f"\t{msg}\n\n")
        if msg == "签到成功！":
...
def watching_sign():
    try:
        ...
        _r = x.get(url=host + f"/_CheckIn/MB/TeachCheckIn.aspx?classid={Course.class_id}&temps=0&checktype=1&isrefresh=0&timeinterval=0&roomid=0&match=", timeout=10)
        if _r.status_code == 200 and 'HFChecktype' in _r.text:
            ...
            if HFCheckInID not in Course.check_list:
                if HFChecktype == '1':
                    ...
                    if HFRoomLongitude is not None and HFRoomLatitude is not None:
                        if int(HFSeconds) <= int(seconds_entry.get()):
...
def is_login():
    ...
    _r = x.get(host + "/AppCode/LoginInfo.ashx", data="Action=checklogin", headers=headers, timeout=10)
    if _r.status_code == 200:
        if _r.json()["msg"] == "1":
            return True
        else:
...
if __name__ == '__main__':
    ...
    tk.Label(tab_frame1, text="剩余倒计时X秒后签到", font=('宋体', 10)).pack(pady=5)
    seconds_entry = tk.Entry(tab_frame1, font=('宋体', 12), width=5)
